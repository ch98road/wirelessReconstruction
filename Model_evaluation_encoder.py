...
from model.Model_define_pytorch_eff import AutoEncoder, DatasetFolder
import torch
import os
    ...
    with torch.no_grad():
        for i, input in enumerate(test_loader):
            ...
            if i == 0:
                ...
                encode_feature = np.concatenate((encode_feature, output),
                                                axis=0)
    print("feedbackbits length is ", np.shape(encode_feature)[-1])
    np.save(
        '{}/{}/encoder_output.npy'.format(cfg.PROJECT_ROOT,
                                          cfg.MODEL_SAVE_PATH), encode_feature)
