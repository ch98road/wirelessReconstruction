...
import config.config_office_multi_quant as cfg
from tensorboardX import SummaryWriter
import utils.CosineAnnealingWithWarmup as LR
from model.Model_define_pytorch_office_Quant import AutoEncoder, DatasetFolder, NMSE
...
from utils.utils import LogScalar, SaveAutoEncoder, SaveAutoEncoderDistributed, TestOneEpoch, TrainOneEpoch, Score
...
InitWeight(model)
if cfg.PRETRAIN:
    if os.path.exists(cfg.PRE_AUTOENCODER_PATH):
        model_CKPT = torch.load(cfg.PRE_AUTOENCODER_PATH)
        model.load_state_dict(model_CKPT['state_dict'])
        print("weight model_CKPT")
    ...
    elif os.path.exists(cfg.PRE_ENCODER_PATH) and os.path.exists(cfg.PRE_DECODER_PATH):
        LoadWeight(model.encoder, cfg.PRE_ENCODER_PATH)
...
testnmseloss = NMSELoss(reduction='mean').cuda()
...
x_train, _, train_loader = DataLoader(datapath=data_load_address +
                                      ...
                                      distributed=not cfg.use_single_gpu)
...
for epoch in range(cfg.epochs):
    ...
    # model evaluating
    if cfg.use_single_gpu or dist.get_rank() == 0:
        test_loss, y_test = TestOneEpoch(model=model,
                                         test_loader=test_loader,
                                         criterion=testnmseloss,
        ...
        if test_loss < best_loss and cfg.SAVE:
            ...
            # SaveAutoEncoderDistributed(model=model, savePath=savePath)
            best_loss = test_loss
        ...
        print('Test_Score={}'.format(1-test_loss))
