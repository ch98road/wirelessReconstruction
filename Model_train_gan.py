...
for epoch in range(epochs):
    ...
    for i, input in enumerate(train_loader):
        ...
        gan_loss = genLoss(output, input)

        # compute gradient and do Adam step
        optimizer.zero_grad()
        mixed_loss = gan_loss*cfg.LAMDA + loss*(1-cfg.LAMDA)
        mixed_loss.backward()
        optimizer.step()

        # 用于计算后面的loss平均
        loss_lis.append(loss.item())
        gan_loss_lis.append(gan_loss.item())
        mixed_loss_lis.append(mixed_loss.item())

        if i % print_freq == 0:
            print(
                'Epoch: [{0}][{1}/{2}]\tLR: {lr:.6f}\tganLoss {ganloss:.3f}\tMSELoss {MSELoss:.3f}\tmixedLoss {mixedLoss:.3f}'
                .format(epoch,
                        i,
                        len(train_loader),
                        lr=cosineLR.get_lr(),
                        ganloss=gan_loss.item(),
                        MSELoss=loss.item(),
                        mixedLoss=mixed_loss.item()))
    # 计算loss平均
    ave_loss = sum(loss_lis) / len(loss_lis)
    gan_ave_loss = sum(gan_loss_lis) / len(gan_loss_lis)
    mixed_ave_loss = sum(mixed_loss_lis) / len(mixed_loss_lis)
    # 画图
    writer.add_scalar('MSE_LOSS', ave_loss, epoch + 1)
    writer.add_scalar('GAN_LOSS', gan_ave_loss, epoch + 1)
    writer.add_scalar('MIXED_LOSS', mixed_ave_loss, epoch + 1)
