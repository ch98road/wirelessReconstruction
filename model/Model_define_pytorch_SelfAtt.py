...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        conv2out = F.relu(self.conv2(out))
        # selfatt
        assert conv2out.shape[2] % 4 == 0 and conv2out.shape[
            3] % 4 == 0, 'conv2out\'s shape is invalid'
        out = conv2out.view(-1, conv2out.shape[1] * 16, conv2out.shape[2] // 4,
                            conv2out.shape[3] // 4)
        attout = self.att(out)
        # 展平
        conv2out = conv2out.view(-1, 32768)
        attout = attout.view(-1, 32768)
        # fc+sig
        out = self.fc(conv2out * attout)
        out = self.sig(out)
