...
class CRBlock(nn.Module):
    def __init__(self, in_planes=16, mid_planes=128, stride=1, groups=1):
        super(CRBlock, self).__init__()
        self.path1 = nn.Sequential(OrderedDict([
            ('conv3x3', ConvBN(in_planes, mid_planes, 3)),
            ('relu1', nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ('conv1x9', ConvBN(mid_planes,
             mid_planes, [1, 9], stride, groups)),
            ('relu2', nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ('conv9x1', ConvBN(mid_planes, mid_planes,
             [9, 1], stride=stride, groups=groups)),
        ]))
        self.path2 = nn.Sequential(OrderedDict([
            ('conv1x5', ConvBN(in_planes, mid_planes, [1, 5])),
            ('relu', nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ('conv5x1', ConvBN(mid_planes, mid_planes,
             [5, 1], stride=stride, groups=groups)),
        ]))
...
class Encoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        self.encoder1 = nn.Sequential(OrderedDict([
            ...
            ("relu1", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("conv1x7_bn", ConvBN(32, 32, [1, 7], 1, 32)),
            ("relu2", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("conv7x1_bn", ConvBN(32, 32, [7, 1], 1, 32)),
        ]))
...
class Decoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True, mid_planes=32):
        ...
        decoder = OrderedDict([
            ...
            ("relu", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("CRBlock1", CRBlock(in_planes=16, mid_planes=mid_planes, stride=1, groups=mid_planes)),
            ("CRBlock2", CRBlock(in_planes=16, mid_planes=mid_planes, stride=1, groups=mid_planes)),
            ("CRBlock3", CRBlock(in_planes=16, mid_planes=mid_planes, stride=1, groups=mid_planes)),
            ("CRBlock4", CRBlock(in_planes=16, mid_planes=mid_planes, stride=1, groups=mid_planes)),
            ("CRBlock5", CRBlock(in_planes=16, mid_planes=mid_planes, stride=1, groups=mid_planes)),
            ("CRBlock6", CRBlock(in_planes=16, mid_planes=mid_planes, stride=1, groups=mid_planes)),
            ("CRBlock7", CRBlock(in_planes=16, mid_planes=16, stride=1, groups=1)),
        ])
