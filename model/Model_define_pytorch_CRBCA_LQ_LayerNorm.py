...
class CRBlock(nn.Module):
    def __init__(self, in_planes=16, mid_planes=128):
        ...
        self.path1 = nn.Sequential(OrderedDict([
            ...
            ('conv1x9', ConvLN(mid_planes, mid_planes, [1, 9])),
            ('relu2', nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ('conv9x1', ConvLN(mid_planes, mid_planes, [9, 1])),
        ]))
        self.path2 = nn.Sequential(OrderedDict([
            ('conv1x5', ConvLN(in_planes, mid_planes, [1, 5])),
            ('relu', nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ('conv5x1', ConvLN(mid_planes, mid_planes, [5, 1])),
        ]))
