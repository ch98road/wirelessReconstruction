...
class Encoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        self.encoder1 = nn.Sequential(OrderedDict([
            ...
            ("relu1", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("conv1x7_bn", ConvBN(32, 64, [1, 7])),
            ("relu2", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("conv7x1_bn", ConvBN(64, 32, [7, 1])),
        ]))
...
class Decoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        decoder = OrderedDict([
            ...
            ("relu", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("CRBlock1", CRBlock(in_planes=16, mid_planes=32)),
            ("CRBlock2", CRBlock(in_planes=16, mid_planes=32)),
            ("CRBlock3", CRBlock(in_planes=16, mid_planes=32)),
            ("CRBlock4", CRBlock(in_planes=16, mid_planes=32)),
            ("CRBlock5", CRBlock(in_planes=16, mid_planes=32)),
            ("CRBlock6", CRBlock(in_planes=16, mid_planes=32)),
        ])
