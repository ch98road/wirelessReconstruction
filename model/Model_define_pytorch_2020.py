...
class CRBlock(nn.Module):
    def __init__(self):
        ...
        self.path2 = nn.Sequential(OrderedDict([
            ('conv1x5', ConvBN(32, 256, [1, 5])),
            ('relu', nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ('conv5x1', ConvBN(256, 256, [5, 1])),
        ]))
        self.conv1x1 = ConvBN(256 * 2, 32, 1)
...
class Encoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        self.encoder1 = nn.Sequential(OrderedDict([
            ("conv3x3_bn", ConvBN(2, 512, 3)),
            ("relu1", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("conv1x7_bn", ConvBN(512, 512, [1, 7])),
            ("relu2", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("conv7x1_bn", ConvBN(512, 512, [7, 1])),
        ]))
        self.encoder2 = ConvBN(2, 512, 3)
        self.encoder_conv = nn.Sequential(OrderedDict([
            ("relu1", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ("conv1x1_bn", ConvBN(512*2, 2, 1)),
            ("relu2", nn.LeakyReLU(negative_slope=0.3, inplace=True)),
        ]))
...
class Decoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        self.fc = nn.Linear(int(feedback_bits / self.B), 126*128)
        decoder = OrderedDict([
